...
public class ZAUEnterKeyActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void requestEmailVerificationCode() {
        ...
        if(getLocation()) {
            String baseUrl = BaseFunctions.getBaseUrl(mContext, "GetEmailCode", BaseFunctions.MAIN_FOLDER, getUserLat(), getUserLon(), mMyApp.getAndroidId());
            String email = account_email.getText().toString().trim();
            String extraParams = "&email=" + email;
            baseUrl += extraParams;
    ...
    private void requestPhoneVerificationCode() {
        if(getLocation()) {
            String baseUrl = BaseFunctions.getBaseUrl(mContext, "GetPhoneCode", BaseFunctions.MAIN_FOLDER, getUserLat(), getUserLon(), mMyApp.getAndroidId());
            String phoneNumber = account_phone.getText().toString().trim();
            if (!new PhonenumberUtils(mContext).isValidPhoneNumber(phoneNumber)) {
                        ...
                        if (jsonObject.has("status") && jsonObject.getBoolean("status")) {
                            ...
                            if (msgData.startsWith("[{")) {
                                ...
                                if (jsonObject.has("OTP")) {
                                    verifyKey = jsonObject.optString("OTP");
                                } else {
                                    verifyKey = msgData;
                                }
                            }

                            appSettings.setALev(jsonObject.optString("svcPro"));
    ...
    private void resetKey() {
        ...
        if (getLocation()) {
            String baseUrl = BaseFunctions.getBaseUrl(mContext, "resetZA", BaseFunctions.APP_FOLDER, getUserLat(), getUserLon(), mMyApp.getAndroidId());

            final Map<String, String> params = new HashMap<>();

            String extraParams = "&code=" + keyVal + // Two Values are same
                    "&SvcPrvID=" + verifyKey +  // Two Values are same
                    "&email=" + email +
                    "&token=" + appSettings.getDeviceToken() +
    ...
    private void sendCodeToEmail() {
        ...
        if (getLocation()) {
            String baseUrl = BaseFunctions.getBaseUrl(mContext, "sendMail", BaseFunctions.APP_FOLDER, getUserLat(), getUserLon(), mMyApp.getAndroidId());
